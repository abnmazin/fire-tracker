...
import { 
  ...
  UserPlus, Trash2, Phone, Menu, X, MapPin, DatabaseBackup, Loader2
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow p-4 md:p-6 mt-6 border border-gray-100">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
          ...
          {user.role === 'developer' && <button onClick={() => setShowContactsModal(true)} className="text-sm bg-blue-50 hover:bg-blue-100 text-blue-600 px-3 py-1.5 rounded-lg flex items-center font-medium transition-colors w-full sm:w-auto justify-center"><Edit className="w-4 h-4 ml-1" /> تعديل الأرقام</button>}
        </div>
...
function UsersList({ users, setUsers, currentUser, logAction, db, fbUser, appId }) {
  ...
  // حجب الرؤية والتعديل على حساب المبرمج إذا كان الدخول بصفة مسؤول (Admin)
  const canSeePassword = (targetRole) => currentUser.role === 'developer' || (currentUser.role === 'admin' && targetRole !== 'developer');
  const canDelete = (targetId, targetRole) => targetId !== currentUser.id && (currentUser.role === 'developer' || (currentUser.role === 'admin' && targetRole !== 'developer'));
        ...
        <table className="w-full text-right min-w-[500px]"><thead className="bg-gray-50 text-gray-600 text-sm"><tr><th className="p-3">الاسم</th><th className="p-3">الحساب</th><th className="p-3">المرور</th><th className="p-3">الصلاحية</th><th className="p-3 text-center">إجراء</th></tr></thead><tbody className="divide-y divide-gray-100 text-sm">
            {users.map(u => (
              <tr key={u.id} className="hover:bg-gray-50">
                ...
                <td className="p-3"><span className={`px-2 py-1 rounded-full text-[11px] font-bold ${u.role === 'developer' ? 'bg-purple-100 text-purple-700' : u.role === 'admin' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>{u.role === 'developer' ? 'مبرمج' : u.role === 'admin' ? 'مسؤول' : 'مفتش'}</span></td>
      ...
      <div className="md:hidden flex flex-col gap-3">
        {users.map(u => (
          <div key={u.id} className="bg-gray-50 border border-gray-100 rounded-lg p-4 flex flex-col gap-3 relative">
            <div className="flex justify-between items-start"><span className="font-bold text-gray-800">{u.name}</span><span className={`px-2 py-1 rounded-full text-[10px] font-bold ${u.role === 'developer' ? 'bg-purple-100 text-purple-700' : u.role === 'admin' ? 'bg-blue-100 text-blue-700' : 'bg-white border text-gray-700'}`}>{u.role === 'developer' ? 'مبرمج' : u.role === 'admin' ? 'مسؤول' : 'مفتش'}</span></div>
...
function AuditLogsList({ logs, userRole }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 w-full relative z-10">
      ...
      <div className="md:hidden flex flex-col gap-3">
        {logs.length === 0 ? (<div className="p-8 text-center text-gray-500 bg-gray-50 rounded-xl border border-gray-100">لا توجد سجلات.</div>) : (logs.map(log => (
          <div key={log.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col gap-2 relative"><div className="flex justify-between items-start mb-1"><span className="px-2 py-1 bg-gray-100 text-gray-800 rounded-md text-[10px] font-bold border">{log.action}</span><span className="text-gray-400 text-[10px]" dir="ltr">{log.date}</span></div><div className="text-sm font-bold text-blue-700">{log.userName}</div><div className="text-xs text-gray-700 leading-relaxed bg-gray-50 p-2 rounded border border-gray-200">{log.details}</div></div>
        )))}
      </div>
    </div>
  );
...
function DeveloperSettings({ locations, setLocations, auditLogs, setAuditLogs, extinguishers, db, fbUser, appId, logAction, currentUser }) {
  ...
  const clearLogs = async () => {
    ...
    if (db && fbUser) {
      // تفريغ بطيء وبسيط لأن التطبيق صغير
