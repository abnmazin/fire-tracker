...
import { 
  ...
  UserPlus, Trash2, Phone, Menu, X, CheckSquare, MapPin
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="bg-gray-50 flex flex-col md:flex-row font-sans text-right min-h-screen md:h-screen md:overflow-hidden" dir="rtl">
      ...
      {/* طبقة خلفية معتمة تظهر فقط في الموبايل عند فتح القائمة */}
      {isMobileMenuOpen && (
        <div 
      ...
      {/* القائمة الجانبية (تنزلق من اليمين في الموبايل وتكون ثابتة في اللابتوب) */}
      <aside className={`fixed inset-y-0 right-0 z-50 w-64 bg-red-800 text-white flex flex-col shadow-2xl transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex flex-col flex-1 overflow-y-auto">
          <div className="p-4 border-b border-red-700/50 flex flex-col justify-center items-center text-center bg-red-900/30">
            ...
            <span className="text-xs bg-red-700 md:bg-red-900 px-2 py-1 rounded-full mt-2 shadow-sm border border-red-600">
          ...
          <div className="p-4 border-t border-red-700 mt-auto pb-6 md:pb-4">
            <button 
              ...
              className="flex items-center justify-center w-full p-2.5 text-red-200 hover:text-white bg-red-900/50 hover:bg-red-700 rounded-lg transition-colors md:mb-4 font-medium"
            >
      ...
      <div className="flex-1 flex flex-col min-w-0 md:h-screen overflow-hidden">
        ...
        {/* الشريط العلوي الخاص بالموبايل فقط */}
        <header className="md:hidden bg-red-800 text-white p-4 flex justify-between items-center shadow-md shrink-0 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-red-200">نظام إدارة الطفايات</p>
            </div>
          </div>
          <button 
  ...
  return (
    <button 
      ...
      className={`flex items-center w-full p-3 rounded-lg transition-colors ${
        active ? 'bg-red-900 text-white font-medium shadow-inner' : 'text-red-100 hover:bg-red-700 hover:text-white'
      }`}
    >
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="mt-6 text-center text-xs text-gray-400 border-t pt-4">
          <p>حقوق المسجد محفوظة © 2026</p>
          <p dir="ltr" className="mt-1 font-mono">abnmazin.engineer</p>
        </div>
      </div>
    </div>
  );
...
function ExtinguishersList({ extinguishers, setExtinguishers, user, logAction, db, fbUser, appId }) {
  ...
  const [filterLocation, setFilterLocation] = useState('All'); // فلتر الموقع الجديد
  ...
  const [bulkDeleteConfirmData, setBulkDeleteConfirmData] = useState(null); // نافذة الحذف الجماعي
  ...
  const filtered = extinguishers.filter(e => {
    ...
    const matchesLocation = filterLocation === 'All' || e.location === filterLocation; // تطبيق فلتر الموقع
  ...
  const handleBulkDelete = async () => {
    ...
    const extNumbers = extsToDelete.map(e => e.number).join('، ');

    if (db && fbUser) {
      const promises = bulkDeleteConfirmData.map(id => 
        deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'extinguishers', String(id)))
      );
      await Promise.all(promises);
    } else {
      setExtinguishers(prev => prev.filter(e => !bulkDeleteConfirmData.includes(e.id)));
    }

    logAction('حذف طفايات', `تم حذف نهائي لـ (${bulkDeleteConfirmData.length}) طفاية: ${extNumbers}`);
  ...
  // حساب عدد الطفايات المحددة القابلة للترحيل (ليست في كابينة)
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100 relative z-20">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto flex-wrap sm:flex-nowrap items-center">
          ...
          <div className="flex flex-row gap-2 w-full sm:w-auto">
            ...
            {/* فلتر الموقع الجديد بدلاً من الكابينة */}
            <div className="relative flex-1 sm:w-36">
              <MapPin className="w-4 h-4 absolute right-3 top-3 text-gray-400" />
              <select value={filterLocation} onChange={(e) => setFilterLocation(e.target.value)} className="w-full pl-2 pr-8 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 outline-none text-xs sm:text-sm text-gray-600 appearance-none bg-gray-50">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            {selectedIds.length > 0 && canEdit && (
              <>
                ...
                <button 
                  onClick={() => setBulkDeleteConfirmData(selectedIds)} 
                  className="flex-1 sm:flex-none bg-red-100 hover:bg-red-200 text-red-700 px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center transition-colors shadow-sm text-sm whitespace-nowrap"
                  title="حذف المحدد"
                >
                ...
                {/* زر الترحيل */}
                <button 
                  disabled={transferrableCount === 0}
                  onClick={() => setTransferModalData(extinguishers.filter(e => selectedIds.includes(e.id) && !e.inCabinet))} 
                  className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center transition-colors shadow-sm text-sm whitespace-nowrap ${transferrableCount === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white'}`}
                  title={transferrableCount === 0 ? 'الطفايات المحددة مثبتة بكابينة ولا يمكن ترحيلها' : 'ترحيل الطفايات غير المثبتة'}
                >
                  <ArrowRightLeft className="w-4 h-4 ml-1" /> ترحيل ({transferrableCount})
                </button>
              </>
            )}
            {canEdit && (
              <button onClick={() => setShowAddModal(true)} className="flex-1 sm:flex-none bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center transition-colors text-sm whitespace-nowrap shadow-sm">
        ...
        <div className="md:hidden bg-white border border-gray-200 rounded-lg p-3 flex justify-between items-center shadow-sm">
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              ...
              onChange={(e) => {
                if (e.target.checked) setSelectedIds(filtered.map(ext => ext.id)); // التحديد يشمل الكل الآن
                else setSelectedIds([]);
              }}
          ...
          {selectedIds.length > 0 && <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-md font-bold">{selectedIds.length} محدد</span>}
        </div>
      )}
            ...
            <tr>
              {canEdit && (
                <th className="p-3 w-10 text-center">
                  <input type="checkbox" className="w-4 h-4 text-red-600 rounded focus:ring-red-500 cursor-pointer" onChange={(e) => { if (e.target.checked) setSelectedIds(filtered.map(ext => ext.id)); else setSelectedIds([]); }} checked={filtered.length > 0 && selectedIds.length === filtered.length} />
                </th>
              )}
              <th className="p-3">الرقم</th>
              ...
              <tr key={ext.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.includes(ext.id) ? 'bg-red-50' : ''}`}>
                {canEdit && (
                  <td className="p-3 text-center">
                    {/* تمت إزالة disabled من صندوق التحديد لكي تتمكن من حذف طفاية حتى لو كانت بكابينة */}
                    <input type="checkbox" className="w-4 h-4 text-red-600 rounded focus:ring-red-500 cursor-pointer" checked={selectedIds.includes(ext.id)} onChange={(e) => { if (e.target.checked) setSelectedIds([...selectedIds, ext.id]); else setSelectedIds(selectedIds.filter(id => id !== ext.id)); }} />
                  </td>
                )}
                <td className="p-3 font-bold text-gray-800">
                  <div className="flex items-center gap-2">
                    {ext.number}
                    {ext.inCabinet && <span title="موضوعة في كابينة (تمنع الترحيل)" className="bg-gray-200 text-gray-500 p-1 rounded-md"><Archive className="w-3 h-3" /></span>}
                  </div>
                </td>
          ...
          <div key={ext.id} className={`bg-white rounded-xl shadow-sm border flex flex-col gap-3 p-4 transition-colors ${selectedIds.includes(ext.id) ? 'border-red-300 bg-red-50' : 'border-gray-100'}`}>
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                {canEdit && (
                  <input 
                    type="checkbox" checked={selectedIds.includes(ext.id)}
                    className="w-5 h-5 text-red-600 rounded focus:ring-red-500"
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {ext.inCabinet && <span className="bg-gray-200 text-gray-500 p-1.5 rounded-md" title="موضوعة في كابينة"><Archive className="w-3 h-3" /></span>}
                  </div>
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden my-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 bg-gray-50 p-3 rounded border border-gray-200"><input type="checkbox" id="inCabinet" className="w-4 h-4 text-red-600 rounded focus:ring-red-500 cursor-pointer" checked={formData.inCabinet} onChange={e => setFormData({...formData, inCabinet: e.target.checked})} /><label htmlFor="inCabinet" className="text-xs sm:text-sm font-medium text-gray-700 cursor-pointer select-none leading-tight">مثبتة داخل كابينة (تمنع الترحيل)</label></div>
...
// 6. نافذة تأكيد الحذف الجماعي (جديدة)
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden my-auto p-6 text-center">
        ...
        <p className="text-sm text-gray-600 mb-6">
          هل أنت متأكد من رغبتك في حذف <strong className="text-red-600">({count})</strong> طفاية بشكل نهائي؟ لا يمكن التراجع عن هذا الإجراء.
        </p>
        <div className="flex gap-3">
          <button onClick={onConfirm} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-lg font-medium transition-colors shadow-md">
            نعم، احذف
          </button>
          <button onClick={onClose} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2.5 rounded-lg font-medium transition-colors">
            إلغاء
          </button>
        </div>
      </div>
